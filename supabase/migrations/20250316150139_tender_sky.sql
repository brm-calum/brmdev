/*
  # Fix Offer Space Allocations

  1. Changes
    - Add space_id to offer_space_allocations table
    - Update save_offer_allocations function to handle space_id
    - Add validation to ensure space_id is provided
  
  2. Security
    - Maintain existing RLS policies
    - Add validation checks for space existence
*/

-- Add space_id to offer_space_allocations
ALTER TABLE public.offer_space_allocations
ADD COLUMN space_id uuid NOT NULL REFERENCES public.m_warehouse_spaces(id);

-- Update save_offer_allocations function
...
BEGIN
  ...
  LOOP
    ...
    IF (v_space->>'offer_total_cents') IS NOT NULL THEN
    ...
    INSERT INTO offer_space_allocations (
      ...
      (v_space->>'offer_total_cents') IS NOT NULL,
      v_space->>'comments'
    );
  END LOOP;
...
-- Add comment
COMMENT ON FUNCTION public.save_offer_allocations IS 'Saves space and service allocations for an offer, with proper space_id handling';